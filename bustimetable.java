...
public class bustimetable{
	...
	public static void printMatrixToFile(ArrayList<String>stops, int [][] absDistances, OutFile results){
		int len = stops.size();
		//find the longest bus stop string 
		int maxStopNameLen = stops.get(0).length();
		for (String s: stops){
		...
		//2.print out the row headings and data
		for (int i=0; i<len; i++){
			results.write(stops.get(i));//print out the row heading
			//sp before the first number is standard space - row heading
			int sp = (space.length()+4-stops.get(i).length());
			for (int k=0;k<sp;k++){
				results.write(" ");
			...
			for (int j=0; j<len;j++){
				results.write(absDistances[i][j]+"      ");//six spaces between numbers that are 2 digits
				if (absDistances[i][j]<10){ //add an extra space for one digit number
					results.write(" ");
				}
			}
	...
	public static void main (String [] args){
		//check if the user provided an input and output file
		if (args.length!=2){
			IO.writeLine("An input file of data and an output file for results must be provided as arguments");
			System.exit(1);
		}
		//check for error when opening files
		InFile data = new InFile(args[0]);
		if (data.openError()){
			IO.writeLine("cannot open file: "+args[0]);
		}//if
		OutFile results = new OutFile(args[1]);
		if(data.openError()){
			IO.writeLine("cannot open file: "+args[1]);
		}
		...
		//printMatrixToConsole(stops,matrix); //prints the poster to the console
		printMatrixToFile(stops,matrix,results);
	}//main
}